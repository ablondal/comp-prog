...
vector<vector<pt>> lights;
vector<pt> pts, rpts, gpts;
vector<pair<para, int>> paras;
vector<char> vis;

bool pass_light(int i, para a){
	double x_at = a.xfromy(i*1000);
	int j = 0;
	while(j<sz(lights[i]) && lights[i][j].x < x_at) j++;
	if (j % 2 == 0) {
		return false;
	} else {
		return true;
